"""Consumidor RabbitMQ MS-2 — cola ms-periodos.events (token.revoked.v1)."""
...
class Command(BaseCommand):
    help = 'Consume token.revoked.v1 para invalidar JWT en MS-2'
    ...
    def handle(self, *args, **options):
        ...
        queue_name = getattr(settings, 'EVENT_QUEUE_NAME', 'ms-periodos.events')
        retry_queue = f'{queue_name}.retry'
        dlq_queue = f'{queue_name}.dlq'

        self.stdout.write(
            self.style.NOTICE(
                f'MS-2 consumer → {queue_name} @ {bus_config.exchange} '
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error('schema_validation_failed', extra={'error': str(exc)})
