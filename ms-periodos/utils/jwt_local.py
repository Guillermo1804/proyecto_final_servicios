...
def _get_signing_keys_cached(client: PyJWKClient) -> list:
    """Claves JWKS en memoria; si MS-1 no responde, reutiliza cache vigente (stale-while-revalidate)."""
...
def _resolve_signing_key(client: PyJWKClient, token: str):
    """Resuelve clave RS256; si el token no trae kid, usa la unica clave del JWKS."""
    ...
    if len(keys) == 1:
        logger.debug(
            "jwt_kid_missing_single_jwk",
            extra={"default_kid": getattr(keys[0], "key_id", None)},
        )
        return keys[0]
...
def validate_access_token(token: str) -> AuthenticatedUser:
    """
    Decodifica y valida un access token RS256 usando claves del JWKS de MS-1.
    PyJWKClient cachea las claves publicas (no consulta MS-1 en cada request).
    """
...
def get_jwks_cache_stats() -> dict[str, int]:
