...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.NOTICE(
                f'MS-4 outbox relay → {bus_config.exchange} @ {bus_config.host}:{bus_config.port}'
            )
        )
    ...
    def _publish_row(
        ...
        envelope = build_envelope(
            ...
            source_service=getattr(settings, 'SERVICE_NAME', 'ms-calificaciones'),
            payload=raw_payload,
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'published event_id={row.event_id} event_name={row.event_name}'
            )
        )
        return True
