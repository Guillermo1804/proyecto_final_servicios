"""Autorización docente ↔ materia en MS-4."""

from __future__ import annotations

import unicodedata

from apps.core.models import DocenteProjection
...
def usuario_puede_gestionar_materia(
    *,
    ...
    docente_titular = (
        DocenteProjection.objects.filter(docente_id=docente_id_materia).first()
    ...
    docente_usuario = DocenteProjection.objects.filter(usuario_id=usuario_id).first()
    if docente_usuario:
