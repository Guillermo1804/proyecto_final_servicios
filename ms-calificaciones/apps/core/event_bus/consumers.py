"""Handlers idempotentes para proyecciones MS-4."""
...
def handle_user_created(envelope: EventEnvelope) -> None:
    _consume(envelope, 'user_created', proj.upsert_user)


def handle_user_updated(envelope: EventEnvelope) -> None:
    _consume(envelope, 'user_updated', proj.upsert_user)


def handle_periodo_created(envelope: EventEnvelope) -> None:
    _consume(envelope, 'periodo_created', lambda p: proj.apply_periodo(p))
...
def handle_materia_closed(envelope: EventEnvelope) -> None:
    _consume(envelope, 'materia_closed_upstream', proj.mark_materia_closed_upstream)
...
HANDLERS = {
    ...
    'user.created.v1': handle_user_created,
