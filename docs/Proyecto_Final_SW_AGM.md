...
- **Gestión completa de periodos académicos:** crear, editar, activar/desactivar y eliminar periodos. El sistema debe garantizar que solo exista un periodo activo en todo momento.
- **Importación masiva de materias** desde el archivo PDF oficial de programación académica de Secretaría Académica. El microservicio extrae NRC, nombre de materia, sección, docente asignado y horario.
- **Importación masiva del directorio de docentes** desde el PDF institucional. El sistema extrae nombre completo, correo institucional y cubículo.
...
El Docente gestiona el proceso académico de sus materias. Sus funcionalidades abarcan todo el ciclo de evaluación:
...
- **Gestión de materias:** visualización de materias asignadas en el periodo activo. Importación de alumnos desde Excel/CSV con vista previa. Al importar un alumno por primera vez, el sistema envía automáticamente su clave única de acceso por correo.
- **Configuración de ponderaciones** por materia: el docente define categorías (ej. Exámenes 40%, Tareas 30%, Proyectos 20%, Asistencia 10%), con validación de que la suma sea exactamente 100%.
- **Gestión de actividades y calificaciones:** para cada categoría de ponderación, el docente crea actividades específicas y asigna calificaciones individuales o masivas. El concentrado muestra el promedio ponderado real y el redondeado (≥ 0.5 redondea al entero superior; < 0.5 al inferior).
- **Módulo de pase de lista / asistencia QR:** el docente inicia una sesión de 10 minutos. El sistema clasifica la asistencia como **Presente** (primeros 5 min) o **Retardo** (entre 5 y 10 min). La sesión se cierra automáticamente al llegar a cero.
- **Cierre de materia:** al cerrar una materia el sistema notifica por correo a todos los alumnos inscritos. Al imprimir la lista final ya no se permiten más cambios.
- **Exportación de calificaciones finales** en formato Excel (XLS) y PDF para captura en actas institucionales.
- **Historial académico:** materias impartidas por periodo, con estadísticas comparativas entre periodos.
...
- **Dashboard** con mensaje de bienvenida personalizado, nombre, matrícula, tipo de formación y periodo activo.
...
- **Baja de materia:** el alumno puede darse de baja únicamente una vez. La operación es irreversible y genera automáticamente una notificación por correo al docente.
...
Inicio de sesión con correo y contraseña, generación y validación de tokens JWT con expiración, recuperación de contraseña vía correo con enlace de un solo uso, cierre de sesión con invalidación del token. El middleware de autorización verifica en cada solicitud el token y el rol del usuario.

**Módulo 2 – Gestión de Periodos Académicos**  
Crear, editar, activar/desactivar y eliminar periodos académicos. Un periodo contiene nombre, fecha de inicio, fecha de fin y plan de estudios. El sistema valida que solo exista un periodo activo a la vez.

**Módulo 3 – Importación y Gestión de Materias y Docentes**  
El microservicio recibe un archivo PDF oficial y, mediante parsing y procesamiento de texto, extrae automáticamente NRC, nombre de materia, sección, clave, docente asignado y horario. También procesa el PDF del directorio institucional para datos de cada docente. Incluye operaciones CRUD manuales.

**Módulo 4 – Gestión de Alumnos y Ponderaciones**  
Inscripción de alumnos por importación Excel/CSV, envío automático de correos de bienvenida con clave única, visualización del concentrado y configuración de ponderaciones por materia con validación de suma = 100%.

**Módulo 5 – Calificaciones y Actividades**  
El docente crea actividades bajo cada categoría de ponderación, asigna calificaciones individuales o masivas (por importación Excel), y el sistema calcula automáticamente el promedio ponderado. El concentrado final muestra el promedio real y el redondeado según reglas institucionales.

**Módulo 6 – Asistencias QR**  
El alumno genera un código QR dinámico con su información cifrada y marca de tiempo. El docente inicia una sesión de 10 minutos. El backend valida el QR, verifica que no haya sido usado previamente (anti-duplicados), registra la asistencia y determina el estado (**Presente** o **Retardo**).
...
Generación y descarga de reportes en formato Excel (XLS/XLSX) y PDF: lista de calificaciones finales por materia, concentrado de asistencias y estadísticas de rendimiento.

**Módulo 9 – Estadísticas e Historial**  
Estadísticas de asistencias y entregas a nivel individual (alumno) y grupal (materia), tanto del periodo activo como históricas. El docente puede comparar el rendimiento de una misma materia entre diferentes periodos.

---

### 5.4 Arquitectura de Microservicios – Backend (NÚCLEO DEL PROYECTO)

> **NÚCLEO DEL PROYECTO:** El backend debe estar dividido en microservicios genuinamente independientes — procesos separados, puertos distintos, bases de datos propias, Dockerfiles individuales. No es válido tener un único servidor con rutas separadas; cada microservicio debe poder ejecutarse, escalarse y desplegarse de forma completamente autónoma.

#### 5.4.1 Los 7 Microservicios Principales

| Microservicio | Responsabilidad principal | Recursos REST externos | Métodos gRPC internos |
|---|---|---|---|
| **MS-1 Auth & Users** | Autenticación JWT, gestión de credenciales, RBAC, recuperación de contraseña. Centraliza la identidad de todos los usuarios. | `POST /auth/login` `POST /auth/refresh-token` `POST /auth/forgot-password` `POST /auth/reset-password` `GET /auth/me` | `ValidateToken(token) → UserClaims` `GetUserById(userId) → UserProfile` `CheckRole(userId, role) → bool` |
| **MS-2 Periodos & Materias** | CRUD de periodos académicos, importación PDF de programación, gestión del catálogo de materias, validación de periodo único activo. | `GET/POST/PUT/DELETE /periodos` `POST /periodos/importar` `GET /materias?periodo=:id` `GET /materias/:id` | `GetMateriaById(materiaId) → MateriaInfo` `GetMateriasByDocente(docenteId) → [Materia]` `GetPeriodoActivo() → PeriodoInfo` |
| **MS-3 Docentes & Alumnos** | Importación PDF de directorio docente, CRUD de docentes, importación Excel de alumnos por materia, gestión del concentrado, baja de materia. | `POST /docentes/importar` `GET /docentes` `POST /alumnos/importar/:materiaId` `GET /alumnos/materia/:materiaId` `DELETE /alumnos/:id/baja` | `GetAlumnosByMateria(materiaId) → [AlumnoInfo]` `GetAlumnoById(alumnoId) → AlumnoInfo` `IsAlumnoEnMateria(alumnoId, materiaId) → bool` |
| **MS-4 Calificaciones & Ponderaciones** | Gestión de ponderaciones (criterios 100%), registro de actividades por categoría, asignación de calificaciones individuales o masivas, cálculo de promedios ponderados y redondeados. | `GET/POST/PUT /ponderaciones/:materiaId` `POST /actividades` `POST /calificaciones` `POST /calificaciones/importar` `GET /concentrado/:materiaId` | `GetConcentrado(materiaId) → [AlumnoCalif]` `GetPromedioAlumno(alumnoId, materiaId) → float` `GetEstadisticasMateria(materiaId) → Stats` |
| **MS-5 Asistencias QR** | Gestión de sesiones de asistencia (10 min), validación de tokens QR dinámicos con anti-replay, clasificación Presente/Retardo, estadísticas diarias y por sesión. | `POST /sesiones/iniciar` `POST /asistencias/registrar` `DELETE /sesiones/:id/cerrar` `GET /asistencias/:materiaId/hoy` `GET /asistencias/:materiaId/historial` | `GetAsistenciaAlumno(alumnoId, materiaId) → [Asistencia]` `GetEstadisticasAsistencia(materiaId) → Stats` |
| **MS-6 Notificaciones** | Envío de correos transaccionales: clave única al registrar alumno, alerta de baja al docente, aviso de calificación final a alumnos, enlace de recuperación de contraseña. | `POST /notificaciones/bienvenida` `POST /notificaciones/baja` `POST /notificaciones/cierre-materia` `POST /notificaciones/reset-password` | `SendBienvenida(alumnoId, materiaId) → bool` `SendBajaNotif(alumnoId, docenteId) → bool` `SendCierreMateria(materiaId) → bool` |
| **MS-7 Reportes & Estadísticas** | Generación de archivos Excel/PDF de calificaciones finales y asistencias, estadísticas históricas de rendimiento grupal e individual, comparativa entre periodos. | `GET /reportes/calificaciones/:materiaId?formato=pdf\|xls` `GET /reportes/asistencias/:materiaId?formato=pdf\|xls` `GET /estadisticas/docente/:id` `GET /estadisticas/alumno/:id` | `GenerateReport(params) → FileBytes` `GetHistorialDocente(docenteId) → [StatsPeriodo]` |

> **Nota:** los "Recursos REST externos" son las rutas HTTP que el microservicio expone al cliente. Los "Métodos gRPC internos" son los procedimientos que ese microservicio ofrece a otros microservicios para comunicación directa inter-servicio. **Ningún microservicio debe acceder directamente a la base de datos de otro.**
...
gRPC es el protocolo **obligatorio** para la comunicación interna entre microservicios. Utiliza Protocol Buffers (`.proto`) para definir contratos de comunicación de forma estricta.
...
Todos los microservicios deben estar desplegados y accesibles públicamente al momento de la presentación. Se requiere:

- **Dockerizar** cada microservicio con su propio `Dockerfile`.
- **`docker-compose.yml`** en la raíz que permita levantar todo el sistema localmente con un solo comando.
...
Requisitos adicionales de despliegue:
...
- **CORS:** cada microservicio debe configurar CORS para aceptar peticiones únicamente desde los orígenes autorizados.
...
> **Punto extra:** el desarrollo del frontend en Angular 20 no forma parte del proyecto obligatorio. Es una actividad complementaria para los equipos que deseen obtener **+1 punto adicional** sobre su calificación final del semestre. Este punto extra se otorga únicamente si el frontend está completo, funcional, conectado a los microservicios del backend desplegado en producción, y tiene un diseño de interfaz profesional.

El frontend debe desarrollarse como una **Single Page Application (SPA)** en Angular (última versión estable disponible). Consumirá los microservicios del backend mediante HTTP/REST, con autenticación JWT en todos los headers de las peticiones.

#### 5.5.1 Requisitos Técnicos Angular

- **Lazy Loading:** cada módulo (admin, docente, alumno) debe cargarse de forma diferida.
- **Route Guards:** implementar `CanActivate` para proteger rutas según el rol del usuario autenticado.
...
- **Reactive Forms** con validaciones síncronas y asíncronas para todos los formularios.
- **Generación de QR** en el módulo del alumno usando `angularx-qrcode` o equivalente. **Escaneo de QR** en el módulo del docente usando `jsQR` o `zxing-js` con acceso a la cámara (MediaDevices API).
...
- **Diseño completamente responsivo (mobile-first).** El módulo de asistencias QR debe funcionar correctamente desde dispositivos móviles.
...
Todos los entregables son obligatorios para el proyecto base (backend). La ausencia de cualquier entregable obligatorio impacta directamente en la calificación.
...
- Descripción de cómo acceder e interactuar con el sistema (desde frontend o desde Swagger UI / Postman si solo se entrega backend).
...
- **Modelo de datos:** diagrama entidad-relación o de colecciones por cada microservicio, con diccionario de datos.
- **Contratos gRPC:** descripción de cada archivo `.proto` con sus servicios y mensajes definidos.
- **Documentación de la API REST** de cada microservicio: método HTTP, URL, parámetros, body de request, response esperado, códigos de error (como colección Postman o archivo OpenAPI).
...
- Subir a YouTube (puede ser privado con enlace compartible) e incluir la URL en el README.
...
El proyecto final representa el **30% de la calificación final del semestre**. La calificación máxima del proyecto es **100 puntos**.
...
| **5. Calidad del código y repositorio** | 8% | Historial de commits continuo y descriptivo, estructura de carpetas organizada, ausencia de credenciales hardcodeadas, `.env.example` presente, separación clara de capas (rutas, controladores, servicios, modelos). |
...
**+1 punto sobre la calificación final del semestre** para los equipos que entreguen el frontend en Angular 20, completo, funcional, conectado al backend desplegado en producción y con diseño de interfaz profesional.

Ejemplo de impacto: si un alumno tiene 8.5 de calificación final antes de sumar el proyecto, y su equipo entrega el frontend completo y de calidad, la calificación final sería **8.5 + 1 = 9.5**. El punto extra aplica sobre la calificación final del semestre, independientemente del puntaje obtenido en el proyecto base.

### 7.2 Criterios de Penalización

| Penalización | Puntos |
|---|---|
| El backend está implementado como monolito único (un solo servidor, una sola base de datos) | **−20 puntos** |
| La comunicación entre servicios no usa gRPC sino REST directo entre microservicios | **−15 puntos** |
| La aplicación no está desplegada en producción o los enlaces no funcionan al momento de la presentación | **−10 puntos** |
| Cualquier módulo funcional principal que no esté implementado o sea completamente inoperativo | **−5 puntos por módulo** |
| El repositorio tiene menos de 20 commits o fue creado menos de una semana antes de la entrega | **−10 puntos** |
| Plagio o uso de código de proyectos anteriores sin comprensión demostrada | **−10 puntos** |
| Ausencia de cualquiera de los entregables obligatorios | **−5 puntos** |

### 7.3 Rúbrica de la Presentación

La presentación es presencial ante el grupo y el docente.
...
| Dominio técnico de microservicios y gRPC | 35 | El equipo puede explicar sus decisiones de arquitectura, mostrar los archivos `.proto`, describir el flujo de una llamada gRPC entre servicios y responder preguntas técnicas del docente. |
