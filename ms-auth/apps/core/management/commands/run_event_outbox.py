"""Worker que publica eventos pending del outbox hacia RabbitMQ."""
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--once",
            action="store_true",
            help="Procesa un lote y termina (util para pruebas).",
        )
        parser.add_argument(
            "--batch-size",
            type=int,
            default=50,
            help="Maximo de eventos por iteracion.",
        )
        parser.add_argument(
            "--poll-seconds",
            type=float,
            default=2.0,
            help="Espera entre iteraciones.",
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.NOTICE(
                f"Outbox relay iniciado → {bus_config.exchange} @ {bus_config.host}:{bus_config.port}"
            )
        )

        while True:
            published = self._relay_batch(bus_config, batch_size)
            if once:
                break
            time.sleep(poll_seconds if published == 0 else 0.1)

    def _relay_batch(self, bus_config: EventBusConfig, batch_size: int) -> int:
        rows = list(
            EventOutbox.objects.filter(status=EventOutbox.Status.PENDING).order_by("created_at")[
                :batch_size
            ]
        )
        if not rows:
    ...
    def _publish_row(
        ...
        causation_id = meta.get("causation_id") or correlation_id
        ...
        try:
            ...
            logger.exception(
                ...
                extra={"event_id": str(row.event_id), "event_name": row.event_name},
            )
            return False
