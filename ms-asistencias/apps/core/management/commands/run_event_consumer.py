"""Consumidor RabbitMQ MS-5 — cola ms-asistencias.events + DLQ."""
...
class Command(BaseCommand):
    help = 'Consume eventos upstream para proyecciones MS-5 (Fase 7)'
    ...
    def handle(self, *args, **options):
        ...
        queue_name = getattr(settings, 'EVENT_QUEUE_NAME', 'ms-asistencias.events')
        retry_queue = f'{queue_name}.retry'
        dlq_queue = f'{queue_name}.dlq'

        self.stdout.write(
            self.style.NOTICE(
                f'MS-5 consumer → {queue_name} @ {bus_config.exchange} '
