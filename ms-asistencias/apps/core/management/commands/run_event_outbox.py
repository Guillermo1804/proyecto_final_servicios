...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.NOTICE(
                f'MS-5 outbox relay → {bus_config.exchange} @ {bus_config.host}:{bus_config.port}'
            )
        )
    ...
    def _publish_row(
        ...
        envelope = build_envelope(
            ...
            source_service=getattr(settings, 'SERVICE_NAME', 'ms-asistencias'),
            payload=raw_payload,
