...
export interface EvaluacionDetalle {
  ...
  descripcion: string;
  categoria: string;
  categoriaPorcentaje: string | number;
  fecha: string;
  fechaIso: string;
...
export class DashboardService {
  ...
  formatearFechaActividad(fechaIso: string | null): string {
    if (!fechaIso) {
      return 'Sin fecha';
    }
  ...
  private mapProximasEvaluaciones(
    grupos: Array<{ materiaId: number; nombreMateria: string; actividades: ActividadApiDto[] }>,
  ): EvaluacionDetalle[] {
    const hoy = new Date();
    hoy.setHours(0, 0, 0, 0);

    const items: EvaluacionDetalle[] = [];
    ...
    for (const grupo of grupos) {
      for (const actividad of grupo.actividades) {
        ...
        items.push({
          ...
          descripcion: actividad.descripcion || 'Sin descripción registrada.',
          categoria: actividad.categoria_nombre,
          categoriaPorcentaje: actividad.categoria_porcentaje,
          fecha: fechaLabel,
    ...
    return items.sort((a, b) => a.sortKey - b.sortKey).slice(0, 12);
  }
  ...
  private mapInscripciones(
    ...
    const materiasActuales: MateriaActual[] = activas.map((item, index) => {
