"""Consumidor RabbitMQ MS-3 — cola ms-alumnos.events (token.revoked.v1)."""
...
class Command(BaseCommand):
    help = 'Consume token.revoked.v1 para invalidar JWT en MS-3'
    ...
    def handle(self, *args, **options):
        ...
        queue_name = getattr(settings, 'EVENT_QUEUE_NAME', 'ms-alumnos.events')
        retry_queue = f'{queue_name}.retry'
        dlq_queue = f'{queue_name}.dlq'

        self.stdout.write(
            self.style.NOTICE(
                f'MS-3 consumer → {queue_name} @ {bus_config.exchange} '
