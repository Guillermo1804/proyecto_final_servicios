"""Consumidor RabbitMQ MS-7 — cola ms-reportes.events."""
...
from apps.reportes.event_bus.consumers import HANDLERS

logger = logging.getLogger(__name__)

# Bindings topic: periodo.* / materia.* / alumno.* → periodo.# / materia.# / alumno.#
ROUTING_KEYS = [
    'periodo.#',
    'materia.#',
    'alumno.#',
    'actividad.created.v1',
    'calificacion.updated.v1',
    'concentrado.calculado.v1',
    'materia.calificaciones_cerradas.v1',
    'asistencia.registered.v1',
    'asistencia.rejected.v1',
    'qr.session.created.v1',
]


class Command(BaseCommand):
    help = 'Consume eventos upstream para proyecciones analíticas MS-7 (Fase 8)'
    ...
    def handle(self, *args, **options):
        ...
        queue_name = getattr(settings, 'EVENT_QUEUE_NAME', 'ms-reportes.events')
        retry_queue = f'{queue_name}.retry'
        dlq_queue = f'{queue_name}.dlq'

        self.stdout.write(
            self.style.NOTICE(
                f'MS-7 consumer → {queue_name} @ {bus_config.exchange} '
